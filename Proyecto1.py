...
class TerrenoCultivo(Tiempo):
    ...
    def sembrar_cultivo(self, fila, columna, cultivo):
        if 0 <= fila < self.filas and 0 <= columna < self.columnas:
            if isinstance(self.terreno[fila][columna], Cultivos):
                ...
                nuevo_cultivo = Cultivos(tiempo.dias, cultivo.nombre, cultivo.tiempo_brote,
                                         cultivo.tiempo_crecimiento + tiempo.dias, cultivo.tiempo_maduracion,
                                         cultivo.productos)
        ...
        if 0 <= fila < self.filas and 0 <= columna < self.columnas:
            ...
            if isinstance(cultivo, Cultivos) and cultivo.etapa == 'Semilla':
                if not cultivo.regado:
                    cultivo.regado = True
                    print(
                        f'Se ha regado y comenzado el crecimiento del cultivo de {cultivo.nombre} en la parcela {fila + 1},{columna + 1}.')
                else:
                    print(
                        f'El cultivo de {cultivo.nombre} en la parcela {fila + 1},{columna + 1} ya ha sido regado en la etapa de "Brote".')
            else:
        ...
        if 0 <= fila < self.filas and 0 <= columna < self.columnas:
            ...
            if isinstance(cultivo, Cultivos):
                if cultivo.plagas:
                    ...
                    print(
                        f'Se han tratado las plagas en el cultivo de {cultivo.nombre} en la parcela {fila + 1},{columna + 1}.')
                else:
        ...
        if 0 <= fila < self.filas and 0 <= columna < self.columnas:
            ...
            if cultivo != '-':
                if cultivo.etapa == 'Cosecha':
                    ...
                    print(
                        f'Se han cosechado {cantidad_productos} productos de {cultivo.nombre} en la parcela {fila + 1},{columna + 1}')
    ...
    def mostrar_terreno(self):
        ...
        for i in self.terreno:
            print('+----' * self.columnas + '+')
            print('| ' + '  | '.join(
                emojis[cultivo.nombre] if isinstance(cultivo, Cultivos) else str(cultivo) for cultivo in i) + '  |')
        print('+----' * self.columnas + '+')
        ...
        for fila in range(self.filas):
            for columna in range(self.columnas):
                ...
                if isinstance(cultivo, Cultivos):
                    ...
                    print(
                        f'Fila: {fila + 1}, Columna: {columna + 1} | Cultivo: {cultivo.nombre} | Etapa: {cultivo.etapa} | Estado: {estado_regado} | Plagas: {plagas_info}')
...
class Mejoras:
    def __init__(self, terreno, animal):
        self.animal = animal
...
while r:
    ...
    if opciones == '1':
        ...
        print("2: Aumento del corral de animales")
        ...
        if op == '2':
            ...
            if opcion == '2':
                ...
                if inventario_jugador.oro >= coste:
                    ...
                    print("Nuevas dimensiones:")
                    terreno.mostrar_terreno()
                    print("el nuevo costo de la siguiente mejora sera:", coste)
                else:
                    print("No tiene suficiente dinero para poder realizar la mejora de filas")
                    print("el costo de su mejora es de:", coste)
